...
export function Header() {
  ...
  return (
    <header className="w-full bg-background border-b border-border/50 sticky top-0 z-50">
      {/* Main Header Row */}
      <div className="container mx-auto px-4 sm:px-6 py-3 md:py-4 flex items-center gap-1.5 sm:gap-4 max-w-full">
        {/* Menu Toggle */}
        <Button
          variant="ghost"
          size="icon"
          className="shrink-0 hover:bg-muted/50 rounded-full w-9 h-9 sm:w-10 sm:h-10"
        ...
        <div className="flex-1 relative flex items-center min-w-0">
          <Search className="absolute left-2.5 sm:left-3 w-4 h-4 sm:w-5 sm:h-5 text-muted-foreground pointer-events-none" />
          <input
            type="text"
            placeholder="Search..."
            className="w-full min-w-0 h-9 sm:h-11 pl-8 sm:pl-11 pr-3 sm:pr-4 rounded-full border border-border/80 bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm placeholder:text-muted-foreground/60 shadow-sm"
          />
        </div>
          ...
          >
            <Link href="/watchlist">
              ...
              {isMounted && watchlistCount > 0 && (
                <span className="absolute top-1 right-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#38bdf8] text-[9px] font-bold text-white border border-background">
          ...
          >
            <Link href="/cart">
              ...
              {isMounted && itemCount > 0 && (
                <span className="absolute top-1 right-1 flex h-5 w-5 items-center justify-center rounded-full bg-[#38bdf8] text-xs font-bold text-white border border-background">
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-background border-t border-border z-50 flex items-center justify-around h-14 px-2 pb-safe shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
        ...
        <Link href="/watchlist" className="relative flex flex-col items-center gap-1 text-muted-foreground hover:text-primary transition-colors py-1 px-3">
          ...
          {isMounted && watchlistCount > 0 && (
            <span className="absolute top-0 right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-[#38bdf8] text-[9px] font-bold text-white border border-background shadow-sm">
              {watchlistCount > 9 ? "9+" : watchlistCount}
            </span>
          )}
        </Link>
